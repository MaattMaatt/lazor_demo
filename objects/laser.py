...
class Laser:
    ...
    def update(self,thisboard,all_points,boardlen,boardht):
        ...
        if boardx < 0 or boardy < 0 or boardx > boardlen-1 or boardy > boardht-1:
            self.killed = 1 # laser is destroyed if it goes beyond board
        else:
            ...
            if thisblock.kill == 1:
                ...
                # (direction unchanged if it goes through)
            else:
                if thisblock.refract == 1: # will report info for new laser to be generated when refracted
        ...
        return all_points,new_laser # return no info to new_laser if we dont refract

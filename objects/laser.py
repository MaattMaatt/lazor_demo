...
class Laser:
    ...
    def update(self,thisboard,all_points,boardlen,boardht):
        ...
        newpt2 = None
        newdir = None
        if boardx < 0 or boardy < 0 or boardx > boardlen or boardy > boardht:
            self.killed = 1
        else:
            # decide how we interact based on block properties, update laser with "newpt"
            thisblock = Block(thisboard[boardx][boardy]) # make block objects only as needed to save time
            if thisblock.kill == 1:
                self.killed = 1
        ...
        return new_laser # return None if we dont refract

# # test stuff
# thisboard = [['C','o','o'],['o','o','o'],['o','o','o']]
# pthit = [Point([2,3])]
# l = Laser([0,3],[1,-1])
# print l.update(thisboard,pthit,3,3)
# print 'stuff:'
